from constraints import ConstraintReport, Constraint
from nodes import Polynomial, Monomial
...
class AppendRule(Rule):
    ...
    def _build_phrase(self, nodes:list) -> Polynomial:
        ...
        for deprel, child in original.children.items():
            if [s for s in starred_deprels if deprel.startswith(s)]: # exists s s.token. deprel starts with it
                deprel = deprel + AppendRule.STAR # add another star to it so it won'token match anymore
...
#             # for now we keep 'self' in the childrens list because the constraints expect this sh*token 
